...
from physical spot models \citep[e.g. \texttt{SOAP 2.0}][]{dumusque14} although I am unaware
...
distribution which I refer to as its \emph{linear representation}. \\
...
which we now insist be a positive correlation, is qualitatively apparent as a low value
...
of a $k=5$ dimensional space which includes three new Gaussian random variables which
...
which includes the familiar covariance amplitude $a$. The periodic kernel
also contains a coherence parameter $\Gamma$ and a periodic timescale $P$ within the sinusoidal term.
With this covariance kernel, measurements which are separated by an integer number of $P$ will be highly
correlated. \\
...
which computes time series of the photometric, RV, and line shape parameters resulting from
