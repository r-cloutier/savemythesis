\chapter{Confirmation of the radial velocity super-Earth K2-18c with HARPS and CARMENES}
...
\abstract{In an earlier campaign to characterize the mass of the transiting temperate super-Earth
  ...
  RV time-series---including a set of 31 previously unpublished HARPS measurements---to investigate
...
Their RV time-series also exhibited a strong additional signal
...
photometry and the aforementioned spectroscopic indicators. All of these ancillary time-series were used
...
independently analyse the  HARPS and CARMENES RV time-series and their joint time-series.
...
\citetalias{sarkis18} created a synthetic RV time-series with the maximum a posteriori (MAP) solution for
...
\subsection{Detecting the 9-day signal in synthetic RV time-series}
...
time-series containing a variety of injected physical signals,
plus a white noise term with standard deviation equal to the mean
RV measurement precision of that time-series.\footnote{ 3.60, 3.08, and 3.37 \mps{} for HARPS, CARMENES, and
  their joint time-series respectively.} We consider four flavours of injected physical signals of increasing
complexity: i) K2-18c only, ii) K2-18b and c, iii) both planets plus correlated noise
due to stellar activity, and iv) K2-18b and stellar activity. The last time-series, which does not contain an injected
...
For each synthetic RV time-series we compute the Bayesian generalized Lomb-Scargle periodogram
\citep[GLSP;][]{mortier15} from which we isolate the probability of a sinusoidal function with the period of
K2-18c ($P_c=8.962$ days) being present in our synthetic time-series; p($P_c|$RV).
...
The synthetic time-series containing K2-18b and stellar activity are not included in Fig.~\ref{k2182fig:9vKcNrv} as
they were consistently seen to result in p($P_c|$RV)$\ll 1$\%, thus indicating that the $P_c$ did not arise with any
significance when not explicitly added to the time-series. The ordinate values in
Fig.~\ref{k2182fig:9vKcNrv} are the median probabilities derived from a set of 50 synthetic time-series realizations
...
As expected, because the $P_c$ periodic signal is injected into each synthetic time-series,
...
time-series. It is true that as the complexity of the synthetic time-series
...
time-series. This shows that with the CARMENES time-sampling 
the strength of the $P_c$ periodic signal is less prominent in the GLSP than with the HARPS or
joint time-sampling. With any of the three types of synthetic time-series, the strength
...
\begin{figure*}
  %\sidecaption
  %\includegraphics[width=0.65\hsize]{9vKcNrv_bkgd.png}%
  \hspace{-0.65\hsize}%
  \begin{ocg}{fig:Hoff}{fig:Hoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:Hon}{fig:Hon}{1}%
  %\includegraphics[width=0.65\hsize]{9vKcNrv_harps.png}%
  \end{ocg}
  \hspace{-0.65\hsize}%
  \begin{ocg}{fig:Coff}{fig:Coff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:Con}{fig:Con}{1}%
  %\includegraphics[width=0.65\hsize]{9vKcNrv_carmenes.png}%
  \end{ocg}
  \hspace{-0.65\hsize}%
  \begin{ocg}{fig:joff}{fig:joff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:jon}{fig:jon}{1}%
  %\includegraphics[width=0.65\hsize]{9vKcNrv_joint.png}%
  \end{ocg}
  \hspace{-0.65\hsize}%
  \caption{\emph{Left column}: Probability of the injected periodic signal at $P_c=8.962$ days existing 
    in synthetic RV time-series, as a function of the injected semi-amplitude $K_c$ with time-sampling
    ...
    RV time-series are considered and contain K2-18c only (\emph{top row}), K2-18b and c (\emph{middle row}),
    ...
    in synthetic RV time-series, with fixed $K_c=4.63$ \mps{,} as a function of the number of RV measurements.}
...
To investigate this possibility, we again compute p($P_c|$RV) in our synthetic RV time-series, but for
random subsets  of each time-series and with an increasing number of RV measurements 
$N_{\text{RV}} \in [10,N_f]$ where $N_f$ is the full size
of each RV time-series.\footnote{$N_f =$ 75, 58, and 133 for HARPS, CARMENES, and their joint time-series,
  respectively.} When creating these synthetic times-series, $K_c$ is fixed to its MAP value of 4.63 \mps{.}
The smoothed probability curves for each synthetic time-series and each WF are shown in the right
...
WFs than with CARMENES alone. Here we focus on the probability of $P_c$ when the HARPS and CARMENES time-series
contain the same number of measurements.
When both time-series are equal to the size of the full CARMENES WF (i.e. $N_{\text{RV}}=58$), the
...
Overall we see that the probability of the $P_c$ periodic signal existing in time-series with the sampling of HARPS,
CARMENES, or their joint time-series, is systematically lowest with CARMENES. By the nature of this experiment we
...
false alarm probability in the GLSP of the full CARMENES time-series \citepalias{sarkis18}. This is confirmed in the
...
\begin{figure*}
  \centering
  %\includegraphics[width=\hsize]{periodograms1_bkgd.png}%
  \hspace{-\hsize}%
  \begin{ocg}{fig:linesoff}{fig:linesoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:lineson}{fig:lineson}{1}%
  %\includegraphics[width=\hsize]{periodograms1_lines.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \caption{Bayesian generalized Lomb-Scargle periodograms for various subsets of the published CARMENES and HARPS RVs 
    with one of a pair of possible weighting schemes. The details of the time-series shown in each panel are
    ...
    is seen at high relative probability in all but the full CARMENES RV time-series from \citetalias{sarkis18}
...
We proceed by calculating the probability of $P_c$ existing within various subsets of the full CARMENES time-series
...
\begin{figure}
  \centering
  %\includegraphics[width=\hsize]{probs9.png}
  \caption{Probability of the proposed K2-18c periodic signal $P_c=8.962$ days existing within the
...
\citetalias{sarkis18} provided their contemporaneous spectroscopic time-series of the CARMENES `full', 
blue, and red RVs, as well as time-series of the chromospheric  H$\alpha$ index and the three Ca$_{\text{II}}$ infrared
triplet line indices. Inspection of these time-series does not reveal any obvious reason for why the three
...
\begin{figure}
  \centering
  %\includegraphics[width=\hsize]{glsp_bluereddiff_bkgd.png}%
  \hspace{-\hsize}%
  \begin{ocg}{fig:linesoff}{fig:linesoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:lineson}{fig:lineson}{1}%
  %\includegraphics[width=\hsize]{glsp_bluereddiff_lines.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \caption{Bayesian generalized Lomb-Scargle periodograms of the blue (\emph{top}), red (\emph{middle}), and 
    ...
    The 9-day signal is seen in the first two time-series, but at a lower probability in the blue likely due to the
...
The full HARPS time-series is spanned by three separate observing seasons
...
activity index (Na D) in all HARPS spectra following \cite{astudillodefru17b}. The Na D time-series is shown in the lower
...
indicators such as the H$\alpha$ index, although its time-series is not depicted in Fig.~\ref{k2182fig:temporal}.

\begin{figure*}
  \centering
  %\includegraphics[width=\hsize]{temporal_bkgd.png}%
  \hspace{-\hsize}%
  \begin{ocg}{fig:vertoff}{fig:vertoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:verton}{fig:verton}{1}%
  %\includegraphics[width=\hsize]{temporal_vert.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \begin{ocg}{fig:textoff}{fig:textoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:texton}{fig:texton}{1}%
  %\includegraphics[width=\hsize]{temporal_text.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \caption{\emph{Top row}: Bayesian generalized Lomb-Scargle periodograms of the HARPS RVs in the three observing
    ...
    $= 38.6$ days). \emph{Bottom row}: Sodium doublet time-series as measured by HARPS. The coloured regions/markers
...
especially true when RV time-series span many stellar rotation cycles.
...
Here we analyse a variety of RV time-series from either the HARPS
...
amplitude. When modelling the joint HARPS+CARMENES time-series we therefore have 19 model parameters.
...
harmonics. By training our GP on ancillary time-series we empirically constrain the covariance structure of the
...
between the various time-series considered and are summarized in Table~\ref{k2182table:priors}. In each
...
Here we model the full HARPS time-series in the same way  as was done for the CARMENES RVs in
...
Here we model the joint RV time-series of the 106 HARPS plus the 55 CARMENES RVs.
...
spectrograph's time-series individually but their ratio is nearly preserved.
...
analysed time-series. This may be due to the lack of a long-term near-linear trend in the stellar activity
...
and two-planet models and for all input time-series considered.
...
corresponding time-series. Overall, we find that the explicit values of the Bayesian
evidence favour the two-planet model for {all} the time-series considered. However, the dispersion in calculated
...
\input{chapters/k2182_modelcomparison}


We recall the $\sim$ 5.5-day signal seen in the K2-18c and residual GLSPs of the RV time-series containing
...
two-planet model for the same input time-series. For the third planet
...
using the full joint HARPS+CARMENES time-series yields a 3-2 Bayes factor of $\sim 0.8$. Therefore, by the effective
...
\item We computed two sets chromatic HARPS RVs. The 9-day
  signal is seen in both time-series and at a significantly higher probability in the red HARPS RVs where stellar
...
in the reduced CARMENES RV time-series. It is important to highlight the importance of basing RV planet
...
in the input time-series (e.g. weighting schemes). Although significant peaks in a GLSP are useful for the initial
identification of periodic signals in unevenly sampled time-series, conclusions regarding their actual existence and origin
should not be made solely based on their FAP. Accurate and simultaneous modelling of all signals present in a
time-series is required to determine accurate model parameters of planets and activity. Furthermore,
...
posterior PDFs for parameters of interest between the individual HARPS, CARMENES, and their joint RV time-series. In this way
we can ensure that the planetary solutions from the time-series of the two spectrographs  are consistent with each other 
and with their joint time-series. For instance, we compare the resulting marginalized poster PDFs of $K_b$ and
$K_c$ obtained with each time-series in Fig.~\ref{k2182fig:postK}. It is evident that the MAP $K_b$ solutions are nearly
equivalent when measured with any of the three time-series. Similarly, MAP $K_c$ values are consistent at the $1\sigma$
level, albeit with more dispersion than the $K_b$ PDFs given the comparatively large uncertainties in the K2-18c
ephemeris.

\begin{figure}
  \centering
  %\includegraphics[width=\hsize]{Kpdf_bkgd.png}%
  \hspace{-\hsize}%
  \begin{ocg}{fig:HKoff}{fig:HKoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:HKon}{fig:HKon}{1}%
  %\includegraphics[width=\hsize]{Kpdf_H.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \begin{ocg}{fig:CKoff}{fig:CKoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:CKon}{fig:CKon}{1}%
  %\includegraphics[width=\hsize]{Kpdf_C.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \begin{ocg}{fig:jKoff}{fig:jKoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:jKon}{fig:jKon}{1}%
  %\includegraphics[width=\hsize]{Kpdf_j.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \caption{The 1D marginalized posterior PDFs of the K2-18b and c semi-amplitudes from analyses
    of the
    ...
    \ToggleLayer{fig:jKon,fig:jKoff}{\protect\cdbox{joint (\emph{black})}} RV time-series. The
...
\appendix
\section{Iterative radial velocity time-series and GLSP figures from Sect.~\ref{k2182sect:correlated}}
...
\begin{figure*} % DONE
  \centering
  %\includegraphics[width=0.77\hsize]{analysisC_bkgd.png}%
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:MCoff}{fig:MCoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:MCon}{fig:MCon}{1}%
  %\includegraphics[width=0.77\hsize]{analysisC_models.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:LCoff}{fig:LCoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:LCon}{fig:LCon}{1}%
  %\includegraphics[width=0.77\hsize]{analysisC_lines.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \caption{Results of our RV analysis of the 55 CARMENES RVs that are known not to significantly
    suppress the apparent 9-day signal seen with HARPS. The RV time-series and their corresponding GLSP
...
\begin{figure*} % DONE
  \centering
  %\includegraphics[width=0.77\hsize]{analysisH106_bkgd.png}%
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:MH106off}{fig:MH106off}{0}%
  \end{ocg}%
  \begin{ocg}{fig:MH106on}{fig:MH106on}{1}%
  %\includegraphics[width=0.77\hsize]{analysisH106_models.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:LH106off}{fig:LH106off}{0}%
  \end{ocg}%
  \begin{ocg}{fig:LH106on}{fig:LH106on}{1}%
  %\includegraphics[width=0.77\hsize]{analysisH106_lines.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \caption{Similar to Fig.~\ref{k2182fig:analysisC}, but for the full HARPS WF containing 106 RVs.
    The rms of the residual time-series assuming a one- and two-planet model are
...
\begin{figure*} %DONE
  \centering
  %\includegraphics[width=0.77\hsize]{analysisHC161_bkgd.png}%
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:MHCoff}{fig:MHCoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:MHCon}{fig:MHCon}{1}%
  %\includegraphics[width=0.77\hsize]{analysisHC161_models.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:LHCoff}{fig:LHCoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:LHCon}{fig:LHCon}{1}%
  %\includegraphics[width=0.77\hsize]{analysisHC161_lines.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \caption{Similar to Fig.~\ref{k2182fig:analysisC}, but for the 161 joint HARPS+CARMENES RVs.
...
\begin{figure*} % DONE
  \centering
  %\includegraphics[width=0.77\hsize]{analysisHCoverlap_bkgd.png}%
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:MHCOoff}{fig:MHCOoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:MHCOon}{fig:MHCOon}{1}%
  %\includegraphics[width=0.77\hsize]{analysisHCoverlap_models.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \begin{ocg}{fig:LHCOoff}{fig:LHCOoff}{0}%
  \end{ocg}%
  \begin{ocg}{fig:LHCOon}{fig:LHCOon}{1}%
  %\includegraphics[width=0.77\hsize]{analysisHCoverlap_lines.png}%
  \end{ocg}
  \hspace{-0.77\hsize}%
  \caption{Similar to Fig.~\ref{k2182fig:analysisC}, but for the 84 joint HARPS+CARMENES RVs
